...
generate_unique_random() {
	num=$(shuf -i 1-1000000 -n 1)
	is_unique=true
	for existing_num in $numbers; do
		if [ "$existing_num" -eq "$num" ]; then
			is_unique=false
			break
		fi
	done
...
echo "\033[94m╔════════════════════════════════════════════════════════════════════════╗\033[0m"
echo "\033[94m║\033[0m               \033[95mWelcome to \033[0m\033[32;4mTestinette\033[0m \033[95m, by ilyanar 🌸\033[0m  \033[94m			 ║\033[0m"
echo "\033[94m╚════════════════════════════════════════════════════════════════════════╝\033[0m\n"
...
then
	...
	if ! expr "$count" : '^[0-9]*$' >/dev/null; then
		printf "\033[31mBad format.\033[0m\n"
		exit 1
	fi
	generate
...
then
	...
	CHECK=$(./push_swap $numbers | ./srcs/checker_linux $numbers)
fi
#
# grep the time
